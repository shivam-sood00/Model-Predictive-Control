...
#Single shooting position tracking/parking MPC
#Author: Shivam Sood
#Date: 11/08/2021
...
class Turtlebot3Mpc(Node):
	...
	*******************************************************************************"""
	...
	def mpc(self):
		...
		OPT_variables = ca.vertcat(U.reshape((-1, 1)))
		...
		lbg = ca.DM.zeros(((n_states-1)*(N), 1))
		ubg = ca.DM.zeros(((n_states-1)*(N), 1))
		
		lbg[:,0] = -ca.inf     # X lower bound
		ubg[:,0] = ca.inf     # X lower bound
		...
		lbx[1: n_controls*N: n_controls] = w_min      
		ubx[1: n_controls*N: n_controls] = w_max
		...
		#print(args['p'])
		args['x0'] = ca.vertcat(ca.reshape(self.u0, n_controls*N, 1))
