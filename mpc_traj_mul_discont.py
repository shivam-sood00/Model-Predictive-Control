...
#Multiple shooting discrete trajectory MPC
#Author: Shivam Sood
#Date: 14/08/2021
...
while t_x<=2*pi:		#Initialize the path to be followed
	points_list.append((cos(t_x)-1,sin(t_x)))
	t_x += 0.005
...
class Turtlebot3Mpc(Node):
	...
	*******************************************************************************"""
	...
	def mpc(self):
		...
		# a=0
		distance,index = spatial.KDTree(points_list).query((x_cur,y_cur))
		for k in range(0,N):
			if (index+1)<len(points_list):
				x_ref,y_ref = points_list[index][0],points_list[index][1]
				theta_ref = atan2((points_list[index+1][1]-points_list[index][1]),(points_list[index+1][0]-points_list[index][0]))
				u_ref, omega_ref = 5,5
				#print("woweee =", theta_ref)
			else:
				x_ref,y_ref = points_list[-1][0],points_list[-1][1]
				theta_ref = atan2((points_list[-1][1]-points_list[-2][1]),(points_list[-1][0]-points_list[-2][0]))
				u_ref, omega_ref = 0,0
				print("WOOOO")
			index+=1
			#print("x = ", x_ref, "y = " ,y_ref)
			# for_plot_x.append(x_cur)
			# for_plot_y.append(y_cur)
		...
		"""
		with open('x', 'w') as f:
			for item in for_plot_x:
				f.write("%s\n" % item)

		with open('y', 'w') as g:
			for item in for_plot_y:
				g.write("%s\n" % item)		
		"""
